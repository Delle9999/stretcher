...
AddEventHandler('stretchermod:SpawnItem', function(key)
  ...
  if v ~= nil then
    ...
    if v.type == 'veh' then
      ...
      nuvarendebil = "stretcher"
      RequestModel(GetHashKey(nuvarendebil))
      while not HasModelLoaded(GetHashKey(nuvarendebil)) do 
          RequestModel(GetHashKey(nuvarendebil))
          Wait(1)
      end
      local bil2 = CreateVehicle(GetHashKey(nuvarendebil), pos, false, false)
      SetEntityAsMissionEntity(bil2, true, true)
    elseif v.type == 'prop' then
      pos = pos + vector3(0.0,0.0,-1.0)
      SpawnObject(v.hash, pos)
    end
  end
end)
    ...
    if DoesEntityExist(closestObject) then
      ...
      if WarMenu.IsMenuOpened('hopital') then
        ...
        if WarMenu.Button(Config.Language.go_out_bed) then
          ...
          exports['mythic_notify']:SendAlert('inform', 'Du har nu forladt stretcheren')
        end
